...
- saya sebenarnya belum menerapkan tema di aplikasi yang saya buat. Tapi kalau nantinya mau pakai, mengatur tema di Flutter itu bakal bikin tampilan aplikasi lebih konsisten dan gampang diatur. Dengan ThemeData, saya bisa setting warna utama, teks, dan gaya visual secara global di MaterialApp, jadi nggak perlu styling tiap widget satu-satu. Flutter juga ada pilihan tema terang dan gelap yang bisa otomatis menyesuaikan sama pengaturan perangkat. Selain itu, saya bisa set style default buat widget seperti AppBar dan Button biar semuanya seragam. Jadi, meskipun sekarang belum diterapkan, kalau dipakai nantinya, tema bakal sangat membantu menjaga konsistensi desain.
...
-  Input Data oleh Pengguna: Data diinput melalui form Flutter menggunakan widget seperti TextFormField. Data yang diinput disimpan ke variabel atau model Dart.
- Mengirim Data ke Server: Data dikirim melalui HTTP POST menggunakan CookieRequest.postJson atau http.post. Data dikirim dalam format JSON ke endpoint API Django.
- Pemrosesan di Django: Django menerima data dari Flutter dan memvalidasinya. Data kemudian disimpan ke database jika valid.
- Mengambil Data dari Server: Flutter menggunakan HTTP GET untuk meminta data dari endpoint API. Django mengirimkan data dalam format JSON ke Flutter.
- Menampilkan Data: Flutter mem-parsing JSON menjadi objek model Dart. Data yang sudah diproses ditampilkan pada UI menggunakan widget seperti ListView atau GridView.
...
```
import 'package:essentialgear_mobile/screens/login.dart';
...
- Saya kemudian membuat halaman detail untuk setiap item yang ada pada list_productentry dengan cara membuat screen baru yang dinamakan gear_detail. gear_detail akan berfungsi menampilkan detail model saya lainnya yaitu menampilkan size dan shade dari produk dan juga tombol back untuk kembali ke page list_productentry. Untuk menampilakn detail kurang lebih membuat kode berikut
...
- Untuk integrasi form flutter dengan django, saya memodifikasi file views pada django main app dengan menambahkan fungsi create_gear_flutter dan memanggilnya di dalam gearentry_form.dart pada proyek flutter saya dan lanjut melakukan penyesuaian fungsionalitas lainnya di file tersebut.
